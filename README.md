...
- `app/admin/page.tsx`：内部审核页面（查看预约并执行“已通过”）
...
- `POST /api/visitors`：提交预约并写入 Supabase `visitors` 表。
- `GET /api/admin/visitors`：查询全部预约记录。
- `PATCH /api/admin/visitors/:id/approve`：将状态从待审核更新为已通过。
